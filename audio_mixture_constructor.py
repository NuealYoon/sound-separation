...
class AudioMixtureConstructor(object):
    ...
    def force_delay_on_signal(self, signal, delay):
        if delay >= 0:
            return signal[delay:]
        else:
            return signal[:delay]
    ...
    def construct_mic_signals(self,
        ...
        # naive way in order to force a delay for DUET algorithm
            ...
            for (s, sr) in source_signals:
                tmp=s[:int(self.mixture_duration * fs)]
                cropped_signals.append(tmp)
            ...
            #     'm2': [self.force_delay_on_signal(s,  int(m2_delays[i]))
            #            for (i, s) in enumerate(cropped_signals)]
            # }
            m1_tmp = []
            m2_tmp = []
            for (i, s) in enumerate(cropped_signals):
                m1_tmp.append(self.force_delay_on_signal(s, int(m1_delays[i])))
                m2_tmp.append(self.force_delay_on_signal(s, int(m2_delays[i])))
            mic_signals = {'m1': m1_tmp, 'm2': m2_tmp}
        return mic_signals

    def get_tf_representations(self,
                               mixture_info):
        """!
        This function constructs the mixture for each mic (m1, m2) in the following way:
        ...
        # m1을 구한 코드를 어떻게 풀어야 할지 모르겠다.
        m1_time_delay_list = []
        m1_time_delay_old = 0
        m2_time_delay_old = 0
        for i in np.arange(n_sources):
            m1_time_delay = positions['amplitudes'][i] * mic_signals['m1'][i]
            m2_time_delay = positions['amplitudes'][i] * mic_signals['m2'][i]

            m1 = m1_time_delay + m1_time_delay_old
            m2 = m2_time_delay + m2_time_delay_old

            m1_time_delay_old = m1_time_delay
            m2_time_delay_old = m2_time_delay
            # m1_time_delay_list.append(m1_time_delay)
